...
class Machine(models.Model):
    ...
    transmission_model = models.ForeignKey(TransmissionModelReference, related_name='transmission_models', on_delete=models.CASCADE)
    transmission_serial_number = models.CharField(max_length=255)
    drive_axle_model = models.ForeignKey(DriveAxleModelReference, related_name='drive_axle_models', on_delete=models.CASCADE)
    drive_axle_serial_number = models.CharField(max_length=255)
    steer_axle_model = models.ForeignKey(SteerAxleModelReference, related_name='steer_axle_models', on_delete=models.CASCADE)
    ...
    client = models.ForeignKey(settings.AUTH_USER_MODEL, related_name='machines', on_delete=models.SET_NULL, null=True, blank=True)
    service_company = models.ForeignKey(ServiceCompanyReference, related_name='service_companies', on_delete=models.CASCADE)
...
class TechnicalMaintenance(models.Model):
    ...
    maintenance_type = models.ForeignKey(TechnicalMaintenanceTypeReference, related_name='maintenance_types', on_delete=models.CASCADE)
    maintenance_date = models.DateField()
    operating_hours = models.IntegerField()
    order_number = models.CharField(max_length=255)
    order_date = models.DateField()
    service_company = models.ForeignKey(ServiceCompanyReference, related_name='maintenance_service_companies', on_delete=models.CASCADE)
...
class Claim(models.Model):
    ...
    recovery_method = models.ForeignKey(RecoveryMethodReference, related_name='recovery_methods', on_delete=models.CASCADE)
    used_spare_parts = models.TextField()
    recovery_date = models.DateField()
    downtime = models.IntegerField()  # Calculated as recovery_date - failure_date
    service_company = models.ForeignKey(ServiceCompanyReference, related_name='claim_service_companies', on_delete=models.CASCADE)
